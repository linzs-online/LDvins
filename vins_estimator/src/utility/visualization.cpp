...
ros::Publisher pub_image_segment;
#endif
...
#ifdef SegmentDynamic
    pub_image_segment = n.advertise<sensor_msgs::Image>("image_segment", 1000);
#endif
...
#ifdef SegmentDynamic
// 发布SegeImage frame_id是world
void pubSegmentImage(const cv::Mat &imgSegment, const double t)
{
    std_msgs::Header header;
    header.frame_id = "world";
    header.stamp = ros::Time(t);
    sensor_msgs::ImagePtr imgSegmentMsg = cv_bridge::CvImage(header, "mono8", imgSegment).toImageMsg();
    pub_image_segment.publish(imgSegmentMsg);
}
#endif
