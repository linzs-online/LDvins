...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    */
    ...
    //cv::resize(imSegment, imSegment, cur_img.size(),cv::INTER_LINEAR);
    for (int i = 0; i < int(cur_pts.size()); i++)
        if (imSegment.at<uchar>(cur_pts[i].y, cur_pts[i].x) == 255)
            status[i] = 0;
#endif
...
// 启用动态物体分割功能,
#ifdef SegmentDynamic
        // mask_temp.create(cur_img.size(), cur_img.type());
        // 使用分割模型进行动态行人分割
        // sampleonnx_ptr->infer(cur_img, imSegment);  // 
        // cv::resize(imSegment, imSegment, cur_img.size(),cv::INTER_LINEAR);
        cv::bitwise_not(mask,mask);  // mask是掩码的地方是黑色的，所以先取反
        cv::add(mask, imSegment, mask);
        cv::bitwise_not(mask,mask);  // 上面叠加得到的掩码的地方是白色的，所以再取反
        // cv::imshow("segment", mask);
        // cv::waitKey(0);
#endif
...
#ifdef SegmentDynamic
cv::Mat FeatureTracker::getTrackSegment()
{
    return imSegment;
}
#endif
