...
void readParameters(std::string config_file)
{
    ...
    SOLVER_TIME = fsSettings["max_solver_time"];            //初始化时间
    ...
    // 通过topic记录真值，以便后续的评估
    fsSettings["ground_truth_save_path"] >> GT_SAVE_PATH;
    gt_save_or_not = fsSettings["save_groundTruth"];
    if ( gt_save_or_not == 1 )
    {
        std::cout << "ground truth save path: " << GT_SAVE_PATH << std::endl;
        std::ofstream fout(GT_SAVE_PATH, std::ios::out); //刷新一下文件
        fout.close();
    }
    fsSettings["onnx_model_path"] >> onnx_model_path;
