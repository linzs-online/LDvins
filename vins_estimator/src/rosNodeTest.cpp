...
void ground_truth_callback(const nav_msgs::Odometry &odom_msg)
{
    if ( gt_save_or_not == 1 )
    {
        //std::string gt_savePth = "/home/linzs/vinsOutput/groundTruth.csv";
        ofstream foutCsave(GT_SAVE_PATH, ios::app);
        foutCsave.precision(9);
        foutCsave.setf(ios::fixed,  ios::floatfield);
        foutCsave << odom_msg.header.stamp.toSec() << " ";
        foutCsave.precision(6);
        foutCsave 	<< odom_msg.pose.pose.position.x << " "
...
int main(int argc, char **argv)
{
    ...
    ROS_WARN("onnx to trt eigen success! now waiting for image and imu...");
